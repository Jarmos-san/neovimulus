...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -qq --no-install-recommends \
    build-essential ca-certificates git curl unzip wget
...
RUN wget "https://github.com/neovim/neovim/releases/download/v0.8.3/nvim-linux64.deb" && \
    apt-get install ./nvim-linux64.deb && rm --recursive --force ./nvim-linux64.deb
